...
JL_DLLEXPORT void JL_NORETURN jl_argument_error(const char *str) // == jl_exceptionf(jl_argumenterror_type, "%s", str)
{
...
JL_DLLEXPORT void JL_NORETURN jl_atomic_error(const char *str) // == jl_exceptionf(jl_atomicerror_type, "%s", str)
{
    jl_value_t *msg = jl_pchar_to_string(str, strlen(str));
    JL_GC_PUSH1(&msg);
