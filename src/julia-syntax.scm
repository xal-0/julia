...
(define (expand-import what body)
  (define (expand-path path mod (final #f))
    (let loop ((mod mod)
               (path path))
      ;; (display (string "  expand-path " mod " " path " " final "\n"))
      (cond ((null? path) mod)
            ((and (not mod) (eq? (car path) '|.|))
             (loop '(thismodule) (cdr path)))
            ((and (not mod) (memq (car path) '(Core Base)))
             (loop (car path) (cdr path)))
            ((and (not mod) (symbol? (car path)))
             (display (string "requiring: " (car path) " context: " body "\n"))
             (loop `(call (top require) (thismodule) ',(car path)) (cdr path)))
            ((eq? (car path) '|.|)
             (loop `(call (top parentmodule) ,mod) (cdr path)))
            ((and final (null? (cdr path)))
             (final mod (car path)))
            ((symbol? (car path))
             (loop `(call (top getproperty) ,mod ',(car path)) (cdr path)))
            (else (error "what")))))
  (define (do-import mod sym)
    `(call (core module_import) (thismodule)
           ,mod ',sym ',sym ,(if (eq? what 'import) '(true) '(false))))
  (define (do-using mod)
    `(call (core module_using) (thismodule) ,mod))

  ;; (display (string "body? " body "\n"))
  (cond ((eq? (caar body) ':)
         (let* ((r (make-ssavalue))
                (mod-path (cdadar body))
                (es (map (lambda (p) (expand-path (cdr p) r do-import))
                         (cddar body))))
           `(block (= ,r ,(expand-path mod-path #f))
                   ,.es
                   (latestworld))))
        ((eq? what 'using)
         `(block ,(do-using (expand-path (cdar body) #f))
                 (latestworld)))
        (else
         `(block ,(expand-path (cdar body) #f do-import)
                 (latestworld)))))
...
(define expand-table
  (table
   ...
   (lambda (e)
    ...
    'import (lambda (e) (expand-import (car e) (cdr e)))
    'using (lambda (e) (expand-import (car e) (cdr e)))))
  ...
  (let* ((stuff (compact-ir (lam:body lam)
    ...
    (define (renumber-stuff e)
      (cond ((eq? e UNUSED) (error "Attempted to use slot marked unused"))
            ...
            ((or (atom? e) (quoted? e) (memq (car e) '(using export public global toplevel)))
             e)
            ((ssavalue? e)
