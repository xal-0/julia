...
static const char opts_hidden[] =
    ...
    "                                               sanitizers.\n"
    " --target-use-jlplt={yes*|no}                  When yes, use Julia's lazy ccall linking. Otherwise,\n"
    "                                               ccalls are emitted as native symbols.\n"
;
...
JL_DLLEXPORT void jl_parse_opts(int *argcp, char ***argvp)
{
    ...
    static const struct option longopts[] = {
        ...
        { "target-use-jlplt", required_argument, 0, opt_target_use_jlplt },
        { 0, 0, 0, 0 }
    };
...
restart_switch:
        ...
        case opt_target_sanitize:
            ...
            break;
        case opt_target_use_jlplt:
            if (!strcmp(optarg, "yes"))
                jl_options.target_use_jlplt = 1;
            else if (!strcmp(optarg, "no"))
                jl_options.target_use_jlplt = 0;
            else
                jl_errorf("julia: invalid argument to --target-use-jlplt={yes|no} (%s)", optarg);
            break;
        default:
